...
logging.basicConfig(filename='runtimes2.log', level=logging.INFO, 
                    format='%(asctime)s - %(message)s', datefmt='%d-%b-%y %H:%M:%S', filemode='a')
logging.info('Model name: Frequentist lasso logistic regression')
...
# Define grid search
param_grid = {"C": loguniform(1e-4, 10)}

# Define the logistic regression model
clf = LogisticRegression(penalty='l1', solver='saga', max_iter=1000, verbose=0)
...
results_df.to_csv("results/frequentist_results/Frequentist_lasso.csv", index=False)
