...
logging.basicConfig(filename='runtimes1.log', level=logging.INFO, 
                    format='%(asctime)s - %(message)s', datefmt='%d-%b-%y %H:%M:%S', filemode='a')
logging.info('Model name: Frequentist simple logistic regression')
...
clf = LogisticRegression(penalty='none', solver='saga', max_iter=1000, verbose=0)
...
def train_and_evaluate(state):
    ...
    result = {
        ...
        **{f'beta.{i+1}': coef for i, coef in enumerate(clf.coef_[0])}
    }
...
results_df.to_csv("results/frequentist_results/Frequentist_logistic.csv", index=False)
